...
int main(void)
{
    ...
    while (scanf(" %s", s) != EOF){ 
        ...
        // se a entrada do usuário for igual a insercao a strcmp retorna 0 e a operacao feita será a insercao
            ...
            if(lista_inserir(l, item) == 0){// se ja tiver a palavra na lista
                item_apagar(&item); item = NULL;
                printf("OPERACAO INVALIDA\n");  
            }
        } else if (strcmp(s, "remocao") == 0){
            // se a entrada do usuário for igual a remocao a strcmp retorna 0 e a operacao feita será a remocao
            scanf(" %s", palavra);   
            if(lista_remover(l, palavra) == 0)// se não existir a palavra na lista
                printf("OPERACAO INVALIDA\n");
        } else if (strcmp(s, "busca") == 0){
            // se a entrada do usuário for igual a busca a strcmp retorna 0 e a operacao feita será a busca
            scanf(" %s", palavra);
            ITEM *item = lista_buscar(l, palavra);
            if(item == NULL){ // se não existir a palavra na lista
                printf("OPERACAO INVALIDA\n");  
            } else
                lista_imprimir_palavra(l, palavra);

        } else if (strcmp(s, "alteracao") == 0){
            // se a entrada do usuário for igual a alteracao a strcmp retorna 0 e a operacao feita será a alteracao
            scanf(" %s", palavra);
            scanf(" %s", definicao);
            scanf(" %[^\n]s", definicao);
            if(lista_alterar(l, palavra, definicao) == 0)// se não existir a palavra na lista
                printf("OPERACAO INVALIDA\n");
            
        } else if (strcmp(s, "impressao") == 0){
            // se a entrada do usuário for igual a impressao a strcmp retorna 0 e a operacao feita será a impressao
            char caracter;
            scanf(" %c", &caracter);
